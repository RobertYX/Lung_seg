...
model.load_state_dict(
    torch.load(join(cfg.ModelSavepath, 'backup', 'model_best_{}.pth'.format(model_idx)), map_location=device))
...
if __name__ == '__main__':
    ...
    for idx in tqdm(range(len(image_path))):
        ...
        for ndx in range(region_nums):
            ...
            # 裁切
            P_image.crop(secZ=[bbox[0] - dz, bbox[3] + dz], secY=[bbox[1] - d, bbox[4] + d],
            ...
            # 返还
            label_data = replace_block(ori_block=label_data, input_block=P_out.get_image(),
                                       secZ=[bbox[0] - dz, bbox[3] + dz], secY=[bbox[1] - d, bbox[4] + d],
                                       secX=[bbox[2] - d, bbox[5] + d])
